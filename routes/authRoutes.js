...
//this kicks them into our oauth flow (ask google for user permissions)
    ...
    app.get(
        ...
        passport.authenticate('google'),
        (req,res) => {
            res.redirect('/surveys'); //once you are logged in you are pushed onto this page
        }
    ); //second route handeler which will handle the case when user visits /auth/google/callback //UPDATE VID 83

    app.get('/api/logout', (req, res) => {
        req.logout(); //it takes the users cookie that contains the user id, and it kills the cookie 
        res.redirect('/'); //this will send a proof the the user that they are no longer signed in
